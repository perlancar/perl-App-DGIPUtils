...
my %known_search_types = (
    trademark => {name_id=>'Merek'},
    patent => {name_id=>'Paten'},
    di => {name_id=>'Desain Industri'},
    copyright => {name_id=>'Hak Cipta'},
    gi => {name_id=>'Indikasi Geografis'},
);
...
$SPEC{':package'} = {
    v => 1.1,
    summary => 'Utilities related to Indonesian DGIP / DJKI website, https://dgip.go.id/',
    description => <<'_',

This module contains utilities related to the Indonesian DGIP (Directorate
General of Intellectual Property) a.k.a. DJKI (Direktorat Jenderal Kekayaan
Intelektual) website, https://dgip.go.id/ and its subsites.
...
$SPEC{search_pdki} = {
    v => 1.1,
    summary => 'Search PDKI (Pangkalan Data Kekayaan Intelektual / Intellectual Property Database) via https://pdki-indonesia.dgip.go.id/',
    description => <<'_',
...
By default, if not specified, will search trademarks.

_
        },
        # XXX sorting
        # XXX sorting ascending/descending
        # XXX filter: Ditarik Kembali
        # XXX filter: Didaftar
        # XXX filter: Dimigrasikan
        # XXX filter: Ditolak
        # XXX filter: Dalam proses
        # XXX filter: Dibatalkan
        # XXX filter: by Nomor Permohonan
        # XXX filter: by Nomor Prioritas
        # XXX filter: Berdasarkan Teks: Nama
        # XXX filter: Berdasarkan Teks: Nama Konsultan
        # XXX filter: Berdasarkan Teks: Nama Pemilik
        queries => {
            'x.name.is_plural' => 1,
